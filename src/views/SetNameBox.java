package views;

import javafx.geometry.Pos;
import javafx.scene.Scene;
import javafx.scene.control.Button;
import javafx.scene.control.Label;
import javafx.scene.control.TextField;
import javafx.scene.image.Image;
import javafx.scene.layout.VBox;
import javafx.stage.Modality;
import javafx.stage.Stage;

public class SetNameBox {
	static String taskName = null;
	
	public static String display(String title) {
		Stage box = new Stage();
		box.initModality(Modality.APPLICATION_MODAL);
		box.setTitle(title);
		box.getIcons().add(new Image("file:resources/image/add.png"));
		box.setMinWidth(250);
		Label label = new Label("Enter task name :");
		
		TextField input = new TextField("Enter Task name here");
		Button confirm = new Button("Ok");
		confirm.setOnAction(e->{
			taskName = input.getText();
			box.close();
		});
		Button cancel = new Button("Cancel");
		cancel.setOnAction(e->{
			box.close();
		});
		
		VBox layout = new VBox();
		layout.getChildren().addAll(label,input,confirm,cancel);
		layout.setAlignment(Pos.CENTER);
		Scene scene = new Scene(layout);
		scene.getStylesheets().add("file:resources/css/application.css");
		box.setScene(scene);
		box.showAndWait();
		
		return taskName;
	}
	
	public static void clear() {
		taskName = null;
	}
}
